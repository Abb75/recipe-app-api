...
class RecipeImageUploadTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = get_user_model().objects.create_user('user@paris.fr', 'testpass')
        self.client.force_authenticate(self.user)
    ...
    def test_upload_image_bad_request(self):
        """Test uploading an invlaid image"""
        url = image_upload_url(self.recipe.id)
    ...
    def test_filter_recipes_by_tags(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='Curry thai')
    ...
    def test_filter_recipes_by_ingredients(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='omelette ')
        recipe2 = sample_recipe(user=self.user, title='fish and chips')
        ingredient1 = sample_ingredients(user=self.user, name='eggs')
        ingredient2 = sample_ingredients(user=self.user, name='Fish')
        recipe1.ingredients.add(ingredient1)
        recipe2.ingredients.add(ingredient2)
        recipe3 = sample_recipe(user=self.user, title='Pasta')
